<script lang="ts">
	...
	let isSubmittingEdited = $state(false);
	...
	async function handleEditorConfirm(blob: Blob) {
		showImageEditor = false;
		isSubmittingEdited = true;
		errorMessage = null;

		try {
			const formData = new FormData();

			// Create a file from the edited blob
			const originalName = evaluation?.product_name || 'edited-image';
			const fileName = `${originalName.replace(/[^a-zA-Z0-9]/g, '_')}_edited.jpg`;
			...
			// Redirect to the new evaluation page
			await goto(`/evaluation/${result.evaluation.id}`);
		} catch (err) {
			console.error('Failed to submit edited image:', err);
			errorMessage = err instanceof Error ? err.message : 'Failed to submit edited image';
		} finally {
			isSubmittingEdited = false;
		}
	}
	...
	async function handleDelete() {
		if (!evaluation || isDeleting) return;
		...
		try {
			...
			errorMessage = err instanceof Error ? err.message : 'Failed to delete evaluation';
			showDeleteConfirm = false;
		} finally {
			isDeleting = false;
		}
...
<!-- Image Editor Modal -->
{#if showImageEditor && evaluation?.imageUrl}
	<ImageEditor
...
<!-- Loading overlay when submitting edited image -->
{#if isSubmittingEdited}
	<div class="fixed inset-0 z-50 bg-black/60 flex items-center justify-center">
		<div class="bg-white rounded-xl p-6 shadow-xl flex flex-col items-center gap-4">
			...
			<p class="text-gray-700 font-medium">Submitting edited image...</p>
			<p class="text-sm text-gray-500">A new evaluation will be created</p>
		</div>
	</div>
{/if}
	...
	<div class="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
		<div class="bg-white rounded-xl shadow-xl max-w-sm w-full p-6">
			<div class="flex items-center gap-3 mb-4">
				...
				<h3 class="text-lg font-semibold text-gray-900">Delete Evaluation</h3>
			</div>
			<p class="text-gray-600 mb-6">
				Are you sure you want to delete this evaluation? This will permanently remove the image and all AI analysis results. This action cannot be undone.
			</p>
		...
		<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 relative">
			...
			<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
				...
				<div class="flex items-center gap-2 self-start">
					...
					<button
						...
						disabled={isDeleting || evaluation.status === 'processing'}
						class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
			...
			<div class="border-t border-gray-200 pt-6">
				...
				{:else}
							...
							{#if evaluation?.imageUrl}
								<div class="flex justify-center">
									<div class="relative group">
										...
										<button
											type="button"
											onclick={(e) => { e.stopPropagation(); openImageEditor(); }}
											disabled={isSubmittingEdited}
											class="absolute top-2 right-2 p-1.5 bg-white/90 hover:bg-amber-50 border border-gray-200 hover:border-amber-300 rounded-lg shadow-sm opacity-0 group-hover:opacity-100 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
											title="Edit & Re-evaluate"
											aria-label="Edit image and re-evaluate"
										>
											<svg class="h-4 w-4 text-gray-600 hover:text-amber-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					...
					{:else}
						<!-- Cards View (existing) -->
						<!-- Image with Edit option -->
						{#if evaluation?.imageUrl}
							<div class="flex justify-center mb-6 pb-6 border-b border-gray-200">
								<div class="relative group">
									<button
										type="button"
										onclick={openLightbox}
									...
									>
										<img
											src={evaluation.imageUrl}
											alt={evaluation.product_name || 'Product label'}
											class="max-h-48 rounded-lg border border-gray-200 shadow-sm object-contain transition-transform group-hover:scale-[1.02]"
										/>
										<div class="absolute inset-0 bg-black/0 group-hover:bg-black/10 rounded-lg transition-colors flex items-center justify-center">
											<span class="opacity-0 group-hover:opacity-100 transition-opacity bg-black/50 text-white px-3 py-1.5 rounded-full text-sm flex items-center gap-1.5">
												<svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
